...
.Code Beispiel
----
include::{javaSourceDir}/de/renatius/tdd/example/ExampleOneUnit.java[tags=contains,indent=0]
----

[source,java]
.Test Code Beispiel
----
include::{javaTestDir}/de/renatius/tdd/example/ExampleOneUnitTest.java[tags=contains,indent=0]
----
...
Abhängigkeiten, d.h. ihr müsst nach dem ihr 1 Test ändert auch darauf aufbauende Tests anpassen.
...
(Es macht keinen Spass einen Unittest debuggen zu müssen, nur weil ein externer Dienst ein Fehler hat.)
...
von Cloud-Computing in Configuration-Stores aus.
...
. test_create_employee_valid_id
. test_create_employee_null_id_throws_exception
. test_create_employee_negative_id_throws_exception
. test_create_employee_duplicate_id_throws_exception
...
Wenn einige deiner Testfälle erwarten, dass die Ausnahmen von der Anwendung ausgelöst werden. Versuche, das Abfangen
...
=== Schreiben Sie keine eigenen catch-Blöcke, die nur existieren, um einen Test nicht zu bestehen
...
Gehe nicht davon aus, dass ein bestimmter Testfall auch ein anderes Szenario testet, den ies fügt Mehrdeutigkeit hinzu.
Schreibe stattdessen für jedes Szenario einen weiteren Testfall.

=== Integrieren Sie Testfälle mit Build-Skript

Es ist besser, wenn Sie Ihre Testfälle mit Build-Skripten integrieren können, damit sie automatisch in deiner
Produktionsumgebung ausgeführt werden. Dies erhöht die Zuverlässigkeit der Anwendung sowie des Testaufbaus.
...
werden. Du solltest immer einen Grund für das Abschalten des Testes angeben. Nicht nur für dich, sondern auch für deine
Kollegen.

. `@Disabled("temporarily disabled for migration")`
. `@Disabled("temporarily disabled to deploy hot fiy for bug: #")`

Dauerhaft deaktivierte Test können sicher entfernt werden, deren Nutzen hat sich einfach überlebt und deren
Produktionscode mit großer Wahrscheinlichkeit auch schon geändert.
